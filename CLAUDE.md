...
├── content/               # Jupyter tutorial notebooks + companion PDFs
│   ├── module0_foundations/
│   ├── module1_circuits/
│   ├── module2_snail/
│   ├── module3_dynamics/
...
└── scripts/               # build_all_pdfs.sh, check_notebooks.sh
```
...
Each notebook in `content/` follows these conventions:
...
This runs `jupyter nbconvert --to pdf --execute` on every notebook using the shared template in `scripts/nbconvert_template.tplx`.
...
CI checks that no file under `src/` or `content/` contains any of these strings:
...
4. If the module adds a new tutorial topic, create a notebook stub in `content/moduleN_name/`.
